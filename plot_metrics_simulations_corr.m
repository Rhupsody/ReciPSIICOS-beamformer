...
% Z_total = (method, synch (always 1), corrN, mc, src)
% methods: ReciPSIICOS, none, LCMV, none
%Z_total = load('ZtotalCorr.mat');
%load('ZtotalCorr.mat');
        ...
        for synch = 1:1
            for method = 1:2:3
                for frac = 1:size(Range_frac, 2)
                    Z = squeeze(Z_total(method,synch,corrN,mc,:))';
                    ...
                    if (length(Z_left(Z_left>0)) == 0)|(length(Z_right(Z_right>0)) == 0)
                        r(method, synch, corrN, frac, mc) = NaN;
                        var(method, synch, corrN, frac, mc) = NaN;
                        r_red(method, synch, corrN, frac, mc) = NaN;
                       
                        xyz_vec_trueL(method, synch, corrN, frac, mc,:) = [NaN, NaN, NaN];
                        xyz_vec_estL(method, synch, corrN, frac, mc,:) =   [NaN, NaN, NaN];
                        xyz_vec_trueR(method, synch, corrN, frac, mc,:) = [NaN, NaN, NaN];
                        xyz_vec_estR(method, synch, corrN, frac, mc,:) =   [NaN, NaN, NaN];
                        ...
                        r_red(method, synch, corrN, frac, mc) = (norm(R_red(max_ind_l,:)-R_red(ind_generated_red(1),:))+ ...
                            norm(R_red(max_ind_r,:)-R_red(ind_generated_red(2),:)))/2;
                        
                        r(method, synch, corrN, frac, mc) = (norm(R_red(max_ind_l,:)-R(ind_generated(1),:))+ ...
                            norm(R_red(max_ind_r,:)-R(ind_generated(2),:)))/2;
                   
                        xyz_vec_trueL(method, synch, corrN, frac, mc,:) = R(ind_generated(1),:) ;
                        xyz_vec_estL(method, synch, corrN, frac, mc,:) =  R_red(max_ind_l,:);
                      
                        xyz_vec_trueR(method, synch, corrN, frac, mc,:) = R(ind_generated(2),:) ;
                        xyz_vec_estR(method, synch, corrN, frac, mc,:) =  R_red(max_ind_r,:);
                        ...
                        var(method, synch, corrN, frac, mc) = (sum(Z_left_norm(Z_left_norm>0).*dist_l)+ ...
